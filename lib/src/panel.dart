...
class SlidingUpPanel extends StatefulWidget {
  ...
  /// [panel] will be used.
  final Widget Function(ScrollController sc, ScrollPhysics sp)? panelBuilder;
...
class _SlidingUpPanelState extends State<SlidingUpPanel> with SingleTickerProviderStateMixin {
  ...
  late ScrollPhysics _sp;
  late InnerObject innerObject;
  ...
  void initState() {
    ...
    // scrollPhysics.allowImplicitScrolling
    ...
    _ac = AnimationController(
      ...
      ..addListener(() {
        ...
        if (widget.onPanelClosed != null && _ac.value == 0.0) {
          ...
          // innerObject.setCanScroll(false);
    ...
    _sp = new InnerScrollPhysics(innerObject: innerObject);
    _sc.addListener(() {
      // if (widget.isDraggable && !_scrollingEnabled) {
      //   _sc.jumpTo(0);
      //   print('jumping to 0 ' + _scrollingEnabled.toString());
      // } else{
      //   print('jumping to 7 ' + _scrollingEnabled.toString());
      //
      //   // if(_sc.offset == 0) {
      //   //   _sc.jumpTo(_sc.offset);
      //   // }
      // }
    });
              ...
              children: <Widget>[
                ...
                Positioned(
                    ...
                    child: Container(
                      ...
                      child: widget.panel != null ? widget.panel : widget.panelBuilder!(_sc, _sp),
                    )),
  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    return Listener(
      ...
      onPointerMove: (PointerMoveEvent p) {
        // print('pointerMoveEvent: ' + p.position.dy.toString());
        _vt.addPosition(p.timeStamp, p.position); // add current position for velocity tracking
        _onGestureSlide(p.delta.dy);

        // if (_scrollingEnabled) {
        //   (_sp as InnerScrollPhysics).setScrollAllowed(true);
        // }
      },
  ...
  void _onGestureSlide(double dy) {
    ...
    if (!innerObject.canScroll || !_sc.hasClients) {
      if (widget.slideDirection == SlideDirection.UP)
    ...
    if (_isPanelOpen && _sc.hasClients && _sc.offset <= 0 && dy > 0 && innerObject.canScroll) {
      innerObject.setCanScroll(false);

      if (widget.panelBuilder != null) {
        _sc.jumpTo(0);
      }
    } else if (!_isPanelOpen && _sc.hasClients && _sc.offset <= 0 && !innerObject.canScroll) {
      innerObject.setCanScroll(false);
    } else if(_ac.value == 1.0 && !innerObject.canScroll){
      innerObject.setCanScroll(true);
  ...
  void _onGestureEnd(Velocity v) {
    ...
    if (_isPanelOpen && innerObject.canScroll) return;
    ...
    if (v.pixelsPerSecond.dy.abs() >= minFlingVelocity) {
      ...
      if (widget.panelSnapping && widget.snapPoint != null) {
        ...
        // _sp.createBallisticSimulation(_sc.position, visualVelocity);
...
class InnerScrollPhysics extends BouncingScrollPhysics {
  final InnerObject innerObject;

  const InnerScrollPhysics({
    ScrollPhysics? parent,
    required this.innerObject,
  }) : super(parent: parent);

  @override
  InnerScrollPhysics applyTo(ScrollPhysics? ancestor) {
    return InnerScrollPhysics(parent: buildParent(ancestor), innerObject: innerObject);
  }

  @override
  Simulation? createBallisticSimulation(ScrollMetrics position, double velocity) {
    if ((velocity.abs() >= tolerance.velocity || position.outOfRange) && innerObject.canScroll) {
